...
const ItemList = ({ fetchItemByInputAsync, itemData,
  ...
  useEffect(() => {
    ...
    //Get the brand list out the in the input list instead of making another call 
    //loop through the itemData to get the brand list
    let unique = [];
    itemData.map((item, index) => {
      //console.log(item.brands.name);
      if (!unique.includes(item.brands.name)) {
        unique.push(item.brands.name);
    }
    })
    //console.log(unique);
    setBrandList(unique);
                      ...
                      return (<div key={index}>
                        <Card style={{ width: "18rem" }} className="box">
                          <Card.Img
                            ...
                            //src={`${import.meta.env.VITE_MIDDLEWARE}/itemsImages/${item.imageUrl}`}
                            src={import.meta.env.VITE_S3 === "True" ?
                              `${item.imageUrl}`
                              :
                          ...
                          <Card.Body>
                            ...
                            <Card.Text className="text-primary">
                              {item.onlineprice} Rs  Quantity {item.online}
                            </Card.Text>
                            {/* <Card.Text className="text-primary">
